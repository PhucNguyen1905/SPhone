		...
		<table class="table table-bordered table-hover" style="margin-top: 20px;">
			...
			<tbody>
				...
				for ($i = 0; $i < $countOrder; $i++) {
					echo '<tr>
					<th>' . $i + 1 . '</th>
					<td><a href="http://localhost/SPhone/OrderAdmin/detailOrder/' . $data["allOrder"][$i]['id'] . '">' . $data["allOrder"][$i]['fullname'] . '</a></td>
					...
					if ($data["allOrder"][$i]['status'] == 0) {
						echo '<a href="http://localhost/SPhone/OrderAdmin/updateStatusOrder/' . $data["allOrder"][$i]['id'] . '/1"><button  class="btn btn-sm btn-success" style="margin-bottom: 10px;">Accept</button><a/>
			<a href="http://localhost/SPhone/OrderAdmin/updateStatusOrder/' . $data["allOrder"][$i]['id'] . '/2"><button class="btn btn-sm btn-danger">Cancel</button><a/>';
