...
<div class="product-list-wrapper">
    ...
    <button style="margin-bottom: 5px; margin-right: 5px;
        ...
        <a style="color:white;text-decoration:none" href="http://localhost/SPhone/Home/productList/0">Tất cả</a>
    </button>
    <?php
    ...
    for ($i = 0; $i < $countCategory; $i++) {
        ...
        echo '" type="button" class="btn btn-primary">
                    <a style="color:white;text-decoration:none" href="http://localhost/SPhone/Home/productList/' . $data["allCategory"][$i]["id"] . '">' . $data["allCategory"][$i]["name"] . '</a></button>';
    }
    ...
    <button style="margin-bottom: 5px; margin-right: 5px;
            ...
            ?>
        " type="button" class="btn btn-primary">
        <a style="color:white;text-decoration:none  " href="http://localhost/SPhone/Home/productList/<?= $data["category_id"] ?>/1/1">Giá (Cao &gt; Thấp)</a></button>
    <button style="margin-bottom: 5px; margin-right: 5px;
            <?php
            if ($data["fillter"] == 2)
                echo "background-color:rgb(236, 79, 58)";
            ?>
        " type="button" class="btn btn-primary">
        <a style="color:white;text-decoration:none" href="http://localhost/SPhone/Home/productList/<?= $data["category_id"] ?>/1/2">Giá (Thấp &gt; Cao)</a></button>
    <button style="margin-bottom: 5px; margin-right: 5px;
            <?php
            if ($data["fillter"] == 3)
                echo "background-color:rgb(236, 79, 58)";
            ?>
        " type="button" class="btn btn-primary">
        <a style="color:white;text-decoration:none" href="http://localhost/SPhone/Home/productList/<?= $data["category_id"] ?>/1/3">Tên (A &gt; Z)</a></button>
    <button style="margin-bottom: 5px; margin-right: 5px;
            <?php
            if ($data["fillter"] == 4)
                echo "background-color:rgb(236, 79, 58)";
            ?>
        " type="button" class="btn btn-primary">
        <a style="color:white;text-decoration:none" href="http://localhost/SPhone/Home/productList/<?= $data["category_id"] ?>/1/4">Tên (Z &gt; A)</a></button>
    ...
    <nav aria-label="Page navigation example">
        <ul class="pagination pg-blue justify-content-center">
            <li class="page-item">
                ...
                if ($data["numPages"] > 1) {
                    if ($data["pages"] == 1) {
                        echo    '<a href="http://localhost/SPhone/Home/productList/' . $data["category_id"] . '/1" class="page-link"><i class="fa fa-chevron-left"></i> Previous</a>';
                    } else echo    '<a href="http://localhost/SPhone/Home/productList/' . $data["category_id"] . '/' . ($data["pages"] - 1) . '" class="page-link"><i class="fa fa-chevron-left"></i> Previous</a>';
                    echo '</li>';
                    for ($i = 1; $i <= $data["numPages"]; $i++) {
                        if ($i == $data["pages"]) {
                            echo '<li class="page-item active"><a class="page-link" href="http://localhost/SPhone/Home/productList/' . $data["category_id"] . '/' . $i . '">' . $i . '</a></li>';
                        } else echo '<li><a class="page-link" href="http://localhost/SPhone/Home/productList/' . $data["category_id"] . '/' . $i . '">' . $i . '</a></li>';
                    }
                    echo '<li class="page-item">';
                    if ($data["pages"] == $data["numPages"]) {
                        echo '<a href="http://localhost/SPhone/Home/productList/' . $data["category_id"] . '/' . $data["numPages"] . '" class="page-link"> Next <i class="fa fa-chevron-right"></i></a>';
                    } else echo '<a href="http://localhost/SPhone/Home/productList/' . $data["category_id"] . '/' . ($data["pages"] + 1) . '" class="page-link "> Next <i class="fa fa-chevron-right"></i></a>';
                }
                ?>
            </li>
        </ul>
...
<script type="text/javascript">
    ...
    function addToCard(productId, priceProduct) {
        var action = "add";
        $.ajax({
            url: "http://localhost/SPhone/home/addToCart",
            method: "POST",
            data: {
                action: action,
                productId: productId,
